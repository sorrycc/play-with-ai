...
export const ALGO_GAMES: Record<DecisionGameId, AlgoGame> = {
  tetris: {
    goal: 'Tetris on a 10-wide, 20-tall board. Each call places one falling piece, or puts it in the hold slot. Full rows clear; the game is lost when the stack reaches the top. In versus mode clearing rows sends garbage rows to the opponent (the match picks the table: one per row cleared, or 0/1/2/4 for a single/double/triple/Tetris; garbageSent on each option is the number that applies), and your own clears cancel garbage waiting for you before it lands. Survive, and clear lines.',
    state:
